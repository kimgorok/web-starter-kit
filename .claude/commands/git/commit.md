...
```
/commit                                  # 자동으로 커밋 메시지 생성
/commit feat: 새로운 기능 추가            # 커스텀 메시지로 커밋
/commit fix: 버그 수정                   # 버그 수정 커밋
```

**인자:**
- `$ARGUMENTS`: 사용자 정의 커밋 메시지 (선택사항)
...
$if($ARGUMENTS)
사용자가 지정한 커밋 메시지로 커밋을 생성합니다: "$ARGUMENTS"

현재 변경사항을 확인하고 제공된 메시지로 커밋하겠습니다.
$else
현재 변경사항을 분석하고 자동으로 커밋 메시지를 생성합니다.
